...
# 职责：
#   1. 按环境变量配置 AI 供应商（custom / deepseek / zai 可共存，默认模型优先级 custom > deepseek > zai）
#   2. 替换 opencode.json 中的 MCP token 占位符
#   3. 将已配置变量写入 /etc/profile.d/ai-env.sh（SSH 会话不继承容器环境，此处兜底）
set -euo pipefail
...
# ---- 4. Z.AI GLM Coding Plan（opencode 官方内置供应商）----
...
# ---- 6. 写入 profile.d，保证 SSH 会话可见（幂等：整文件覆盖重写）----
env_vars=(GLM_MCP_TOKEN OPENCODE_CUSTOM_BASE_URL OPENCODE_CUSTOM_API_KEY OPENCODE_CUSTOM_MODEL DEEPSEEK_API_KEY ZAI_API_KEY OPENCODE_MODEL)
{
